...
angular.module('app')
    ...
    .service('global', ['$document', '$q', '$http', '$state', '$translate', '$filter', function ($document, $q, $http, $state, $translate, $filter) {
        ...
        //操作日志
        self.sendLog = function (operation, origin, data, field, status, describe, location, type,cb) {
            ...
            if( data.details === false){
                ...
                for(var key in data){
                    if(origin[key] == undefined || !_.isEqual(origin[key], data[key])){
                        isSendLog = true;

                        before[key] = origin[key];
                        after[key] = data[key];
                        if(field){
                            fields[key] = self.translateByKey(field[key]);
                        }
                    }
                }
            }
            ...
            if(isSendLog){
                var logData = {
                    operation: operation,
                    status: status, // 0 失败，1 成功
                    type: 1, //0 常规日志 1 操作数据日志 2 业务日志
                    content: {
                        describe: self.translateByKey(describe),
                        location: location,
                        before: before,
                        after: after,
                        fields: fields,
